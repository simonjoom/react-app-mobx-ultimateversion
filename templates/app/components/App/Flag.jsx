...
return <IconButton href={"http://"+process.env['SITEPT']+history.createHref(path)} key={ky}  alt="Portugais (PT)" title="Portugais (PT)"><Icon_Flag_BR /></IconButton>;
}else if (FL=='UK'){
path=findroute_thendebug(component,window.__routesuk__,'uk')
return <IconButton href={"http://"+process.env['SITEUK']+history.createHref(path)} key={ky} alt="Ukrainien (UK)" title="Ukrainien (UK)"><Icon_Flag_UK /></IconButton>;
}else{
path=findroute_thendebug(component,window.__routesen__,'en')
return <IconButton href={"http://"+process.env['SITEEN']+history.createHref(path)} key={ky} alt="English (UK)" title="English (UK)"><Icon_Flag_US /></IconButton>;
}
}
